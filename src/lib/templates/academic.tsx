        ...
        <div className="bg-white text-gray-800 font-serif w-full shadow-none print:shadow-none flex flex-col gap-2 text-[10pt] flex-1" style={{ fontFamily: "Georgia, serif" }}>
            ...
            {data.basics && (
                <header className="text-center mb-1 border-b-[1px] border-slate-200 px-12 pt-10 pb-4">
                    <h1 className="text-3xl font-black text-slate-900 mb-2 uppercase tracking-tight">{data.basics.name}</h1>
                    <div className="text-[10pt] text-slate-500 flex justify-center flex-wrap gap-x-5 gap-y-1 items-center italic">
            ...
            {/* Main Content */}
            <div className="space-y-6 px-12 pb-10 pt-4">
                {/* 0. Summary */}
                {data.basics?.summary && (
                    <section>
                        ...
                        <p className="text-[10pt] leading-relaxed text-slate-700 text-justify italic">
                            {data.basics.summary}
                        </p>
                    </section>
                )}
                            ...
                            {data.education.map((edu, i) => (
                                <div key={i}>
                                    <div className="flex justify-between items-baseline mb-0.5">
                                        ...
                                        <span className="text-[9.5pt] text-slate-500 font-bold italic tracking-tight">{edu.startDate} — {edu.endDate || 'Present'}</span>
                                    </div>
                                    <div className="text-[10.5pt] text-slate-600 font-bold">{edu.institution}</div>
                                </div>
                            ))}
                        </div>
                    </section>
                            ...
                            {data.experience.map((exp, i) => (
                                <div key={i}>
                                    <div className="flex justify-between items-baseline mb-0.5">
                                        ...
                                        <span className="text-[9.5pt] text-slate-500 font-bold italic tracking-tight">{exp.startDate} — {exp.endDate || 'Present'}</span>
                                    </div>
                                    <div className="text-[10pt] text-slate-600 font-bold italic mb-2">{exp.company}</div>
                    ...
                    <section>
                        ...
                        <div className="space-y-4">
                            {data.projects.map((proj, i) => (
                                <div key={i}>
                                    <h3 className="text-[11pt] font-bold text-slate-900 tracking-tight mb-1">{proj.name}</h3>
                                    <p className="text-[9.5pt] text-slate-600 leading-relaxed text-justify italic">{proj.description}</p>
                                </div>
                            ))}
                        </div>
                    </section>
                ...
                <div className="grid grid-cols-2 gap-x-12 pt-2">
                    {data.achievements && data.achievements.length > 0 && (
                        <section>
                            <h2 className="text-[9pt] font-black text-slate-900 border-b border-slate-200 pb-1 mb-2 tracking-widest uppercase">Awards</h2>
                    ...
                    {data.certifications && data.certifications.length > 0 && (
                        <section>
                            <h2 className="text-[9pt] font-black text-slate-900 border-b border-slate-200 pb-1 mb-2 tracking-widest uppercase">Certs</h2>
                            <ul className="space-y-2 text-[9pt] text-slate-600 font-medium leading-tight">
