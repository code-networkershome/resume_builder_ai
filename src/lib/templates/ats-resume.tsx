        ...
        <div className="bg-white text-slate-800 font-sans leading-snug w-full flex flex-col gap-2 shadow-none print:shadow-none text-[10.5pt] overflow-hidden flex-1 px-10 py-8">
            ...
            {data.header && (
                <header className="text-center border-b-[1.5pt] border-black pb-2">
                    <h1 className="text-2xl font-bold uppercase tracking-wide mb-0.5 leading-none">{data.header.name || ''}</h1>
                    <div className="flex flex-wrap justify-center gap-x-4 gap-y-0.5 text-[9.5pt] font-medium text-slate-700">
                        ...
                        {data.header.location && data.header.phone && <span className="text-slate-300">•</span>}
                        {data.header.phone && <span>{data.header.phone}</span>}
                        {data.header.phone && data.header.email && <span className="text-slate-300">•</span>}
                        {data.header.email && <a href={`mailto:${data.header.email}`} className="underline decoration-slate-200 underline-offset-4">{data.header.email}</a>}
                    </div>
            ...
            <div className="space-y-3 flex-1">
                ...
                {data.experience && data.experience.length > 0 && (
                    <section className="space-y-1">
                        <h2 className="text-[10.5pt] font-bold uppercase tracking-tight border-b-[1pt] border-black pb-1 mb-1">Experience</h2>
                ...
                {data.projects && data.projects.length > 0 && (
                    <section className="space-y-1">
                        <h2 className="text-[10.5pt] font-bold uppercase tracking-tight border-b-[1pt] border-black pb-1 mb-1">Projects</h2>
                ...
                {/* Skills */}
                {data.skills && data.skills.categories && data.skills.categories.some(cat => cat.skills && cat.category) && (
                    <section className="space-y-1">
                        <h2 className="text-[10.5pt] font-bold uppercase tracking-tight border-b-[1pt] border-black pb-1 mb-1">Expertise</h2>
