    ...
    return (
        <div className="bg-slate-50 text-slate-800 w-full font-sans leading-snug flex flex-col shadow-none print:shadow-none text-[10pt] flex-1">
            ...
            <div className="px-10 pt-8 pb-4">
                <header className="relative py-3 px-5 bg-white rounded-[1.25rem] border border-slate-200 shadow-xl ring-1 ring-slate-100">
                    ...
                    <div className="relative z-10">
                        <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase mb-2 leading-none">
                            {data.header.name}
                        </h1>
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[9pt] font-black uppercase tracking-widest text-slate-400">
                        ...
                        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[9pt] font-black uppercase tracking-widest mt-3">
            ...
            {/* Main Content Grid */}
            <div className="grid grid-cols-12 gap-8 flex-1 px-10 pb-8 mt-4">
                {/* Left Side: Skills & Info */}
                <div className="col-span-4 space-y-3">
                    <section>
                        <h2 className="text-[10pt] font-black uppercase tracking-[0.2em] text-primary mb-2 flex items-center gap-3">
                            Expertise
                        ...
                        <div className="flex flex-wrap gap-2">
                            {(data.skills.categories || []).map((cat, i) => (
                                (cat.skills || "").split(",").map((skill: string, j: number) => {
                                    ...
                                    return (
                                        <span key={`${i}-${j}`} className="px-2 py-1 bg-white border border-slate-100 text-slate-700 rounded-lg text-[8.5pt] font-bold uppercase tracking-wider shadow-sm hover:border-primary/30 transition-colors">
                                            {trimmed}
                                        </span>
                                    );
                                })
                    ...
                    {data.education.length > 0 && (
                        <section>
                            <h2 className="text-[10pt] font-black uppercase tracking-[0.25em] text-primary mb-2 flex items-center gap-4">
                                Education
                                <div className="h-1 w-8 bg-primary/10 rounded-full" />
                            </h2>
                            <div className="space-y-2">
                                {data.education.map((edu, i) => (
                                    <div key={i} className="group bg-white p-2 rounded-xl border border-slate-50 shadow-sm hover:shadow-md transition-shadow">
                                        <div className="font-black text-slate-900 text-[9.5pt] uppercase leading-tight mb-1">{edu.degree}</div>
                                        <div className="text-[8.5pt] text-slate-400 font-bold uppercase">{edu.institution}</div>
                                        <div className="text-[8.5pt] text-primary font-black mt-1 tracking-widest">{edu.duration}</div>
                                    </div>
                                ))}
                            </div>
                        </section>
                ...
                <div className="col-span-8 space-y-3">
                    ...
                    {data.projects.length > 0 && (
                        <section>
                            <h2 className="text-[9.5pt] font-black uppercase tracking-[0.2em] text-primary mb-2 flex items-center gap-3">
                                Signature Work
                                <div className="h-px bg-slate-200 flex-1" />
                            </h2>
                            <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-3 pt-2">
                        {data.achievements.length > 0 && (
                            <section className="bg-white p-3 rounded-[1rem] border border-slate-100 shadow-sm">
                                <h2 className="text-[9.5pt] font-black uppercase tracking-[0.15em] text-primary mb-3">Milestones</h2>
                                <ul className="space-y-2 text-[9pt] text-slate-600 font-medium">
                        ...
                        {data.certifications.length > 0 && (
                            <section className="bg-white p-3 rounded-[1rem] border border-slate-100 shadow-sm">
                                <h2 className="text-[9.5pt] font-black uppercase tracking-[0.15em] text-primary mb-3">Accreditation</h2>
                                <ul className="space-y-2 text-[9pt] text-slate-600 font-medium">
