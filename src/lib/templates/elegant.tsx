...
export const ElegantTemplate: React.FC<{ data: ResumeData }> = ({ data }) => {
    return (
        <div className="bg-white text-slate-800 font-serif leading-snug w-full flex flex-col gap-4 shadow-none print:shadow-none text-[10pt] flex-1" style={{ fontFamily: "Merriweather, serif" }}>
            {/* Header - Truly Elegant & Balanced */}
            <header className="text-center pt-8 pb-10">
                <h1 className="text-4xl font-light text-slate-900 tracking-[0.25em] uppercase leading-none mb-6">{data.header.name}</h1>
                <div className="flex justify-center flex-wrap gap-x-8 gap-y-2 text-[9pt] font-medium uppercase tracking-widest text-slate-500 max-w-2xl mx-auto">
                    {data.header.location && <span>{data.header.location}</span>}
                    {data.header.location && (data.header.phone || data.header.email) && <span className="text-slate-200">|</span>}
                    {data.header.phone && <span>{data.header.phone}</span>}
                    {data.header.phone && data.header.email && <span className="text-slate-200">|</span>}
                ...
                {(data.header.linkedin || data.header.github || data.header.leetcode || data.header.portfolio || (data.header.customLinks && data.header.customLinks.length > 0)) && (
                    <div className="flex justify-center flex-wrap gap-x-6 gap-y-1 text-[8.5pt] font-bold uppercase tracking-[0.1em] text-primary/70 mt-4 italic">
                ...
                <div className="w-16 h-[2px] bg-slate-900 mt-10 mx-auto opacity-30" />
            </header>
            ...
            <div className="space-y-10 px-10 pb-10">
                                ...
                                <div key={i}>
                                    <div className="flex justify-between items-baseline mb-3">
                                        <div className="max-w-[75%]">
                                            ...
                                            <p className="text-[10pt] text-primary font-bold uppercase mt-1 tracking-wider opacity-80">{exp.organization}</p>
                                        </div>
                                        <span className="text-[9pt] font-black text-slate-400 uppercase tracking-widest whitespace-nowrap">{exp.duration}</span>
                                    </div>
                                    <ul className="list-disc list-outside ml-6 space-y-1.5 text-[9.5pt] text-slate-600 leading-normal font-sans font-medium">
                            ...
                            {data.education.map((edu, i) => (
                                <div key={i} className="flex justify-between items-start text-center max-w-2xl mx-auto">
                                    <div className="flex-1 text-right pr-6 pt-1">
                                        <h3 className="text-[11pt] font-bold text-slate-900 uppercase tracking-tight">{edu.degree}</h3>
                                    </div>
                                    <div className="w-2.5 h-2.5 rounded-full bg-slate-100 border border-slate-200 mt-2 shrink-0" />
                                    <div className="flex-1 text-left pl-6">
                                        <p className="text-[10pt] text-slate-500 font-bold uppercase tracking-wide leading-none mb-2">{edu.institution}</p>
                                        <p className="text-[9pt] text-slate-300 font-black uppercase tracking-[0.15em]">{edu.duration}</p>
                                        {edu.cgpa && <p className="text-[8.5pt] text-primary font-black mt-2">GPA {edu.cgpa}</p>}
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-8">
                            {data.projects.map((proj, i) => (
                                <div key={i}>
                                    <div className="flex justify-between items-baseline mb-3">
                                        <div className="max-w-[75%]">
                                            <h3 className="text-[11.5pt] font-bold text-slate-900 uppercase tracking-tight leading-snug">{proj.name}</h3>
                                            <p className="text-[10pt] text-primary font-bold italic mt-1 opacity-70">{proj.techStack}</p>
                                        </div>
                                        {proj.link && <a href={proj.link} className="text-[8.5pt] font-black text-primary uppercase border-b border-primary/20 hover:border-primary transition-colors pb-0.5 tracking-widest whitespace-nowrap italic">Link →</a>}
                                    </div>
                                    <ul className="list-disc list-outside ml-6 space-y-1.5 text-[9.5pt] text-slate-600 leading-normal font-sans font-medium">
                ...
                {data.skills && data.skills.categories && data.skills.categories.some(cat => cat.skills) && (
                    <section>
                        ...
                        <div className="grid grid-cols-2 gap-x-12 gap-y-6 max-w-3xl mx-auto font-sans">
