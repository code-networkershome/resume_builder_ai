    ...
    return (
        <div className="bg-white text-slate-800 font-sans leading-snug w-full flex flex-col gap-3 shadow-none print:shadow-none text-[10pt] flex-1">
            ...
            <header className="border-b-2 border-slate-900 px-10 pt-8 pb-4">
                ...
                <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1 text-[9pt] font-semibold uppercase tracking-widest text-primary">
                            ...
                            {data.projects.map((proj, i) => (
                                <div key={i} className="space-y-1">
                                    <div className="flex justify-between items-baseline">
                                        ...
                                        {proj.link && (
                                            <a href={proj.link} className="text-[9pt] font-semibold text-primary border-b border-primary/20 hover:border-primary transition-all uppercase tracking-widest">
                                                Case Study →
                                            </a>
                                        )}
                                    </div>
                                    <p className="text-slate-500 italic font-semibold tracking-wide text-[9.5pt]">
                                        {proj.techStack}
                                    </p>
                                    <ul className="space-y-0.5 text-[10pt] text-slate-700 font-medium leading-tight">
                            ...
                            {(data.skills.categories || []).map((cat, i) => (
                                cat.skills && (
                                    <div key={i} className="text-[9.5pt]">
                                        <span className="font-semibold text-slate-900 uppercase tracking-wider mr-2">
                                            {cat.category}:
                                        </span>
                                    ...
                                    <div key={i} className="flex justify-between items-baseline">
                                        <div className="space-y-0.5">
                                            ...
                                            <p className="text-[10pt] text-slate-600 font-semibold italic uppercase tracking-wide">
                                                {edu.institution} {edu.cgpa && <span className="text-primary font-semibold not-italic ml-2">• GPA {edu.cgpa}</span>}
                                            </p>
                                        </div>
