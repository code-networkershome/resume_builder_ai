            ...
            <div className="flex flex-1 gap-8">
                ...
                <aside className="w-[30%] border-r border-slate-100 pr-10 pl-10 py-4">
                    <div className="space-y-3">
                        {/* Skills - Bubbles */}
                        <section>
                            <h2 className={`text-[10px] uppercase tracking-widest mb-2 font-black ${c.primary} border-l-4 border-blue-100 pl-3`}>Skills</h2>
                            <div className="space-y-3">
                                    ...
                                    <div key={i}>
                                        ...
                                        <div className="flex flex-wrap gap-2">
                                            {(cat.skills || "").split(',').filter(s => s.trim()).map((s, j) => (
                                                <span key={j} className={`text-[9px] font-bold uppercase px-2 py-1 rounded-full border border-blue-100 bg-white text-slate-600`}>{s.trim()}</span>
                                            ))}
                                        </div>
                                    </div>
                                ))}
                        ...
                        {data.certifications && data.certifications.length > 0 && (
                            <section>
                                <h2 className={`text-[10px] uppercase tracking-widest mb-2 font-black ${c.primary} border-l-4 border-blue-100 pl-3`}>Certifications</h2>
                ...
                <main className="w-[65%] py-4 pr-10">
                    <div className="space-y-3">
                        ...
                        {data.achievements && data.achievements.length > 0 && (
                            <section>
                                <h2 className={`text-[10px] uppercase tracking-widest mb-2 font-black ${c.primary} border-l-4 border-blue-100 pl-3`}>Achievements</h2>
