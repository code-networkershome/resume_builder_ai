...
export const ModernTemplate: React.FC<{ data: ResumeData }> = ({ data }) => {
    return (
        <div className="bg-white text-slate-800 font-sans leading-snug w-full flex flex-col gap-3 shadow-none print:shadow-none text-[10pt] flex-1">
            ...
            {data.header && (
                <header className="mb-2 px-10 pt-8">
                    <div className="flex items-end justify-between border-b-2 border-slate-900 pb-2">
                        <div>
                            ...
                            {data.header.location && <p className="text-slate-500 font-bold mt-1 uppercase tracking-[0.2em] text-[9pt]">{data.header.location}</p>}
                        </div>
                        ...
                        <div className="space-y-3">
                            {data.experience.map((exp, i) => (
                                <div key={i}>
                                    ...
                                    <ul className="space-y-0.5 text-[10pt] text-slate-600 font-medium leading-tight">
